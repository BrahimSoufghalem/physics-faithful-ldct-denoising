"""TransCT -- copied from ldctbench/methods/transct/network.py
...
removed (the original constructor never reads it) and the top-level class was
renamed Model -> TransCT.

IMPORTANT: this architecture hard-codes 512x512 inputs. The space-to-depth
blocks and the `torch.reshape(..., (b, c, 32, 32))` in forward() assume
H = W = 512 (512/16 = 32 tokens per side, 512/32 = 16 for the LF memory).
Train it with --patch-size 512 --val-patch-size 512 and a small batch size;
train.py enforces this. Requires torchvision (GaussianBlur).
"""
