...
Let $x$ be the LDCT input and $B$ any denoising trunk. We define the removed noise $\hat{n} = x - B(x)$ and produce the final output $\hat{x} = x - H(\hat{n};\, x)$, where $H$ is the spectral head. Because the interface requires only the pair $(x, B(x))$, the head attaches to any denoiser --- CNN, GAN generator, or transformer --- without modifying the trunk. The interface is trunk-agnostic; whether the learned trunk--head equilibrium is beneficial must be measured per trunk (Sec.~\ref{sec:crosstrunk}). All main results use a benchmark-faithful RED-CNN trunk as the representative testbed (Sec.~\ref{sec:related}).
...
\begin{enumerate}
  \item \textbf{Exact DC preservation of the residual correction.} The gain applies to non-zero frequencies only, so the head cannot alter the spatial mean of the removed residual: the global HU level of $\hat{x}$ equals that of the trunk output $B(x)$ exactly. Provable, not learned. We state the scope precisely: this guarantees that the \emph{head} introduces no global HU shift; it does not by itself guarantee absolute HU calibration of the final image, since the trunk may carry its own global bias --- that bias is targeted by the HU-bin loss (Sec.~\ref{sec:objective}), not by this constraint.
  \item \textbf{Near-DC freezing.} The first two knots are pinned at $G = 1$, removing the head's capacity to alter large-area regional HU means --- a capacity restriction on the correction, not a correctness guarantee on regional calibration. This constraint was introduced after diagnosing a $-7$\hu{} regional chest soft-tissue bias and faint concentric rings caused by a sharp gain step adjacent to DC (chest soft-tissue bias $-7.0 \rightarrow -3.5$\hu{} after freezing).
  \item \textbf{Radial parameterization.} The curve is 32 scalars --- isotropic, interpretable, directly plottable. The head introduces no new directional frequency content and restricts the correction to an isotropic modulation of the predicted residual; it cannot synthesize image content the way an unconstrained generative decoder can.
...
$L_{\mathrm{NPS}}$ matches the radially averaged log noise-power spectrum of the removed noise $x - \hat{x}$ to that of the reference noise $\mathrm{LDCT} - \mathrm{NDCT}$, penalizing both over-removal and the characteristic low-frequency under-removal hole left by appearance-trained trunks. We say \emph{reference noise} deliberately: the paired residual is the standard supervised noise surrogate, but strictly it contains all differences between the two reconstructions (including reconstruction nonlinearities and minor anatomical or resolution discrepancies), not detector noise alone; treating it as the noise reference is the standing assumption of the paired-supervised setting.
