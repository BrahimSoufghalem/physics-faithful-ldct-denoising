...
\begin{table}[!tbp]
\centering
\caption{Configurations. ``frz-DC 2'' = two near-DC knots frozen at $G{=}1$; ``cent.'' = batch-centering penalty (weight 0.05).}
\label{tab:configs}
...
S1 & static & 0 & 0 & Neutrality check \\
S3b & static, frz-DC 2 & 0.005 & 0.2 & Static head \\
S4 & adaptive, frz-DC 2 & 0.005 & 0.2 & Saturation diagnostic \\
\textbf{S4b} & adapt.\ + cent., frz-DC 2 & 0.01 & 0.2 & \textbf{Proposed} \\
\bottomrule
\end{tabular}}
\end{table}
...
Image quality: PSNR, SSIM, VIF, RMSE\textsubscript{HU}. Physical fidelity (removed noise $= x - \hat{x}$, reference noise $= \mathrm{LDCT} - \mathrm{NDCT}$, central $320 \times 320$ crops):

\begin{enumerate}
  \item \textbf{\npsl{} ($\downarrow$)} --- $L_1$ distance between radially averaged log noise-power spectra of removed vs.\ reference noise; the \textbf{pre-specified primary endpoint} of the study. All formal statistical claims refer to this metric; the remaining metrics are reported as secondary, descriptive evidence.
  \item \textbf{\npsc{} ($\uparrow$)} --- Pearson correlation of the radial NPS curves; spectral shape fidelity independent of magnitude.
  \item \textbf{\stdr{} ($\rightarrow 1$)} --- std of removed noise / std of reference noise; below 1 means residual noise left in the image, above 1 means anatomy erased as ``noise''.
...
Overall aggregates can cancel: C2's overall bone bias of $-2.0$\hu{} hides chest $-18.3$ / abdomen $+14.3$\hu{}. All tables report chest and abdomen separately; overall values are shown for convenience only. Statistical testing: paired per-patient Wilcoxon signed-rank tests on the primary endpoint ($n = 10$, two-sided; a 10/10 sweep yields the exact minimum $p = 0.002$). Seeds are repeated trainings evaluated on the same 10 patients, so we test within each seed rather than pooling: ``30/30'' throughout the paper means 30 patient--seed comparisons (10 patients $\times$ 3 seeds), not 30 independent patients.
